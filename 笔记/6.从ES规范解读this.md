...
1. 规范中属性访问表达式行为解释最后一条 **返回一个 Reference 类型**
...
1. 规范中属性访问表达式行为解释第三条 **调用了 GetValue(rref)**, 所以返回的不是 Reference 类型
...
1. 规范中属性访问表达式行为解释第二条 **调用了 GetValue(lref)**, 所以返回的不是 Reference 类型
