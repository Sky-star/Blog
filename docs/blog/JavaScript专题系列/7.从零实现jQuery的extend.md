...
可以看到，进行深拷贝之后，b2 并没有被覆盖掉。 从上一篇知道深拷贝进行递归拷贝，所以能够进行更深层次的添加和覆盖。
...
在实现上，核心的部分还是跟上篇实现的深浅拷贝函数已知， 如果要复制对象的属性值是一个对象，就递归调用 extend。不过 extend 的实现中，多了很多细节上的判断，比如第一个参数是否是布尔值，target 是否是一个对象，不穿参数时的默认值等。
